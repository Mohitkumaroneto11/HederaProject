...
const { authenticateSocketConnection_ } = require("../middleware/authentication");
module.exports = function routes(route) {
    route.get('/', (request, response) => {
        response.send(`Api server in running (${new Date()})`);
    });
    route.get('/api/game/registation', (request, response) => {
    ...
    route.use('/api/game/registation', async (req, res, next) => {
